...
# Datasets
parser.add_argument('-d', '--data', default='/data/bitahub/Tiny-ImageNet', type=str)
...
parser.add_argument('--n_classes', type=int, default=200, help='num of classes')
...
def main():
    ...
    f=open('/data/bitahub/Tiny-ImageNet/wnids.txt','r')
    for line in f.readlines():
        line=line.strip()
        val_dataset.classes.append(line)
    f.close()
    val_dataset.classes.remove('images')
    val_dataset.class_to_idx = train_dataset.class_to_idx
    f=open('/data/bitahub/Tiny-ImageNet/val/val_annotations.txt','r')
    imgs_list = []
    target_list = []
    for line in f.readlines():
        re_line = re.match("(.*?)\s*?(n\d*?)\s",line)
        imgs_list.append(("/data/bitahub/Tiny-ImageNet/val/images/"+str(re_line.group(1)),val_dataset.class_to_idx[re_line.group(2)]))
        target_list.append(val_dataset.class_to_idx[re_line.group(2)])
    val_dataset.imgs = imgs_list
    val_dataset.samples = imgs_list
    val_dataset.targets = target_list
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print('train_loss: {:.4f}, train_top1_acc: {:.2f}, train_top5_acc: {:.2f}, test_loss: {:.4f}, test_top1_acc: {:.2f}, test_top5_acc: {:.2f}, cong_bf: {:.4f}, cong_af: {:.4f}, mag: {:.4f}'.format(
            train_loss, train_top1_acc, train_top5_acc,
            test_loss, test_top1_acc, test_top5_acc,
            cong_bf, cong_af, mag))

        # uncomment the following line to use tensorboard
        writer.add_scalars('top-1', {'train top-1':train_top1_acc, 'val top-1':test_top1_acc}, epoch+1)
        writer.add_scalars('loss', {'train':train_loss, 'val':test_loss}, epoch+1)
        writer.add_scalars('top-5', {'train top-5':train_top5_acc, 'val top-5':test_top5_acc}, epoch+1)
        logger.append([epoch + 1, train_top1_acc, test_top1_acc, test_top5_acc, cong_bf, cong_af, mag, 
